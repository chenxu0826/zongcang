<template>
  <div id="app">
    ...
    <navheader
      ...
      :toolPlan="toolPlan"
      :toolplanStartTime="toolplanStartTime"
      :toolplanEndTime="toolplanEndTime"
    ...
    methods: {
     ...
     /* 零时互监组取消操作，清空刷卡内容*/
      ...
      alarmHandle:function () {        
        ...
        var alarmHandS = setInterval(function () {//用循环是为了判断是否刷卡
          if(localStorage.getItem("placemanID")==0&&vm.Isopen){//先判断民警ID是否为0 为0表示未登陆 
             vm.alertYHDL=true//用户登录框是否显示                             
          }else if(localStorage.getItem("placemanID")==1&&vm.Isopen){//先判断民警ID是否为1 为1表示未登陆     
               vm.alertYHDL=true                                                     
          }
           else {//报警处理提交
      ...
      alarmGo:function () {
        ...
        if(this.alarmNowPage<this.alarmPages){
          ...
          if(this.alarmList[this.alarmA-1]["EventCode"]==1003){//互监组报警(下页)
            this.isGrup=true;
            this.isPerson=false
        ...
        }else if(this.alarmList[this.alarmA-1]["EventCode"]==1001){//区域入侵报警(下页) {
...
//            alert("请求异常")
     ...
     /*查看区域人员详情*/
      ...
      alertAlarm:function () {  
        ...
        this.alarmNowPage=1
        var vm=this
        if(this.alarmList[0]["EventCode"]==1003) {//第一个报警类型为互监组报警
          this.isGrup=true
          this.isPerson=false
          $.ajax({
              type: "get",
      ...
      allDataInit:function () {
        ...
        $.ajax({
          ...
          jsonp: "callback",
          async: false,
          //data:{"OrgID":localStorage.getItem("OrgID")},//当前监区人数
    ...
    mounted () {
      ...
      vm.ws.onmessage=function(event) {
        ...
        /* 报警信息 */
          ...
          if (alarmNews.OrgID.toUpperCase() == localStorage.getItem("OrgID")) {   
            ...
            if(criminalInfo!=null && criminalInfo!=undefined){ //zyf
      ...
      //已报警数据删除
          ...
          for(let i=0; i<personnel_distribution_rec.length; i++){
            try{
              chartsParms[i] = {
                ...
                CriminalCnt:personnel_distribution_rec[i].CriminalCnt.GeneralManagement + personnel_distribution_rec[i].CriminalCnt.Investigate + personnel_distribution_rec[i].CriminalCnt.LooseManagement + personnel_distribution_rec[i].CriminalCnt.StrictManagement,
        ...
        /* 计划任务-返回数据-4 */
          ...
          if(plan_task.PlanTypeName != '巡更计划'&&plan_task.OrgID.toUpperCase() == localStorage.getItem("OrgID")){
            ...
            if(vm.plan == '工具清点计划'&& plan_task.OrgID.toUpperCase() == localStorage.getItem("OrgID")){
              vm.toolPlan="工具清点计划"
            ...
            } else if(vm.plan == '人员清点计划'&& plan_task.OrgID.toUpperCase() == localStorage.getItem("OrgID")){             
